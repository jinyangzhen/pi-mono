...
const VERSION = pkg.version.padEnd(11);
...
program
    ...
    if (options.port)
        config.update({ port: parseInt(options.port, 10) });
    if (options.host)
        config.update({ host: options.host });
    if (options.agentDir)
        config.update({ agentDir: options.agentDir });
    if (options.workspace)
        config.update({ workspaceDir: options.workspace });
